...
const adminOnly = require('../middlewares/adminOnly'),
    ...
    { authDeniedGroupId } = require('../../config');
...
router.post('/', adminOnly, async (req, res) => {
    ...
    if (sanction.isBan === true) {
        const banGroup = await groupModel.getGroup(authDeniedGroupId);
        if (Array.isArray(banGroup) && banGroup.length > 0) {
            result = await groupModel.createUserGroup(sanction.userId, authDeniedGroupId);
            if (result > 0) {
                ...
                if (result.rowCount > 0) {
                    ...
                    groupModel.deleteUserGroup(sanction.userId, authDeniedGroupId);
            ...
            return res.status(500).json({ message: '영구 불인증 그룹이 config에 지정되어있지 않습니다. 그룹 지정 후 다시 시도해주세요.' });
        }
    } else {
