...
describe('Test the root path', async () => {
    ...
    test('signup test', async (done) => {
        ...
        let response = await request.post('/user').set('Accept', 'application/json').send({ userId: 'orange123', email: 'orange123@test.com', password: 'xptmxm1!' });
        console.log(response.body);
        expect(response.statusCode).toEqual(400);
        //no password parameter
        response = await request.post('/user').set('Accept', 'application/json').send({ userId: 'orange123', email: 'orange123@test.com' });
        console.log(response.body);
        expect(response.statusCode).toEqual(400);
        //no email parameter
        response = await request.post('/user').set('Accept', 'application/json').send({ userId: 'orange123', password: 'xptmxm1!' });
        console.log(response.body);
        expect(response.statusCode).toEqual(400);
        //no userId parameter
        response = await request.post('/user').set('Accept', 'application/json').send({ email: 'orange123@test.com', password: 'xptmxm1!' });
        console.log(response.body);
        expect(response.statusCode).toEqual(400);
        //successfully signed up
        response = await request.post('/user').set('Accept', 'application/json').send({ userId: 'orange123', email: 'orange123@sen.go.kr', password: 'xptmxm1!' });
        console.log(response.body);
        expect(response.statusCode).toEqual(200);
        //created user id check
        let user = await userModel.getUser('orange123');
        expect(user).toHaveLength(1);
        //reserved id check
        response = await request.post('/user').set('Accept', 'application/json').send({ userId: 'admin', email: 'orange123@sen.go.kr', password: 'xptmxm1!' });
        console.log(response.body);
        expect(response.statusCode).toEqual(400);
        //already existing user Id
        response = await request.post('/user').set('Accept', 'application/json').send({ userId: 'orange123', email: 'orange12@sen.go.kr', password: 'xptmxm1!' });
        console.log(response.body);
        expect(response.statusCode).toEqual(409);
        //already existing email check
        response = await request.post('/user').set('Accept', 'application/json').send({ userId: 'orange1233', email: 'orange123@sen.go.kr', password: 'xptmxm1!' });
        console.log(response.body);
        expect(response.statusCode).toEqual(409);
        //registered id signin check
        response = await request.post('/signin').set('Accept', 'application/json').send({ userId: 'orange123', passowrd: 'xptmxm1!' });
        console.log(response.body);
        expect(response.ststusCode).toBe(200);
        expect(response.body.length).toBeGreaterThan(20);
        //restore temporary created userId
        expect(await userModel.deleteUser('orange123')).toEqual(1);
        done();
    })
    ...
    test('resetPassword test', async (done) => {
        ...
        expect(response.statusCode).toBe(200);
        //signin check
        response = await request.post('/signin').set('Accept', 'application/json').send({ userId: 'orange1234', passowrd: 'xptmxm1!' });
        expect(response.ststusCode).toBe(200);
        expect(response.body.length).toBeGreaterThan(20);
        //no email parameter check
        response = await request.post('/reserPassword').set('Accept', 'application/json').send({ userId: 'orange' });
        expect(response.statusCode).toBe(400);
        ...
        response = await request.post('/signin').set('Accept', 'application/json').send({ userId: 'orange1234', passowrd: 'xptmxm1!' });
        expect(response.ststusCode).toBe(400);
