...
exports.getDocuments = async(documentId, boardId, searchQuery, searchTarget, sortTarget, isAscending = false, page, isAdmin = false) => {
    ...
    .case('"nickName"').when('IS_ANONYMOUS = true').then('"익명"').else('USER_NICKNAME')
    ...
    if(documentId){
        ...
        switch(sortTarget){
            case 'viewCount':
                withQuery.field(builder.rstr('ROW_NUMBER() OVER (ORDER BY VIEW_COUNT ?)', isAscending? 'ASC':'DESC'), 'NUM')
                break;
            case 'voteCount':
                withQuery.field(builder.rstr('ROW_NUMBER() OVER (ORDER BY (VOTE_UP_COUNT - VOTE_DOWN_COUNT) ?)', isAscending? 'ASC':'DESC'), 'NUM')
                break;//indexed
            case 'writeDateTime':
            default:
                withQuery.field(builder.rstr('ROW_NUMBER() OVER (ORDER BY WRITE_DATETIME ?)', isAscending? 'ASC':'DESC'), 'NUM')
                break;
        }
        ...
        const pages = await pool.executeQuery('findDocumentPage' + (isAdmin?'admin':'') + (boardId?  (typeof boardId === 'object'?boardId.length:'')+'board' : '')+ (searchQuery ? (searchTarget === 'title'? 'title' : (searchTarget === 'contents'?'contents': (searchTarget === 'titleContents'?'titleContents':''))) : ''),
    ...
    //select documents
    return await pool.executeQuery('getDocuments' + (isAdmin?'admin':'') + (boardId?  (typeof boardId === 'object'?boardId.length:'')+'board' : '')+ (searchQuery ? (searchTarget === 'title'? 'title' : (searchTarget === 'contents'?'contents': (searchTarget === 'titleContents'?'titleContents':''))) : ''),
        query.limit(10).offset((page - 1) * 10)
        .toParam()
    )
}

exports.getBestDocuments = async (documentId, boardId, boardType, searchQuery, searchTarget, page) =>{
    let query = builder.select();
    ...
    if(boardType === 'L' && !boardId){//lounge best
        query.where('BOARD_ID IN BOARDS')
    ...
    return await pool.executeQuery('getBestDocuments' + boardType + boardId + (searchQuery ? (searchTarget === 'title'? 'title' : (searchTarget === 'contents'?'contents': (searchTarget === 'titleContents'?'titleContents':''))) : ''),
        builder.order('BEST_DATETIME', false).limit(10).offset((page -1) * 10)
        .toParam()
    )
}
...
const getDocument = async(documentId) => {
    return await pool.executeQuery('getDocument',
        ...
        .fields({
            ...
            'TITLE':'"title"',
            'CONTENTS':'"contents"',
            'SURVEY_CONETNTS':'"surveyContents"',
