...
router.get('/', checkSignin, async (req, res) => {//get group detail
    let groupType = null;
...
router.post('/', adminOnly, async (req, res) => {//create new group
    ...
    }else if(group.groupIconPath.length > 200){
    ...
    if((typeof result === 'object' && result.error) || result.rowCount === 0){
        ...
        return res.status(200).json({message:`${group.groupName} 그룹을 추가하였습니다.`, groupId:result.groupId});
    }
});
...
router.delete('/:groupId(^[\\d]+$)', adminOnly, async (req, res) => {//delete existing group
