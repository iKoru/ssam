...
router.post('/', adminOnly, async(req, res) => {
    ...
    if (typeof notification.template !== 'string' || notification.reportTypeDescription.length > 2000) {
        return res.status(400).json({ target: 'template', message: '알림 내용 템플릿이 2000자를 넘거나 형태가 올바르지 않습니다.' })
    }
    if (notification.variable1 !== undefined && (typeof notification.variable1 !== 'string' || notification.variable1.length > 100)) {
        return res.status(400).json({ target: 'variable1', message: '알림 내용 변수1이 100자를 넘거나 형태가 올바르지 않습니다.' })
    }
    if (notification.variable2 !== undefined && (typeof notification.variable2 !== 'string' || notification.variable2.length > 100)) {
        return res.status(400).json({ target: 'variable2', message: '알림 내용 변수2가 100자를 넘거나 형태가 올바르지 않습니다.' })
    }
    if (notification.variable3 !== undefined && (typeof notification.variable3 !== 'string' || notification.variable3.length > 100)) {
        return res.status(400).json({ target: 'variable3', message: '알림 내용 변수3이 100자를 넘거나 형태가 올바르지 않습니다.' })
    }
    if (notification.variable4 !== undefined && (typeof notification.variable4 !== 'string' || notification.variable4.length > 100)) {
    ...
    if (notification.target !== undefined && typeof notification.target !== 'number') {
        return res.status(400).json({ target: 'target', message: '알림 내용 타겟의 형태가 올바르지 않습니다.' })
    }
    if(notification.href !== undefined && (typeof notification.href !== 'string' || notification.href.length > 100)){
    ...
    if(notification.groupId){
        ...
        if(Array.isArray(users)){
            ...
            users.forEach(x=>{
                notificationModel.createNotification({...notification, userId:users[i].userId});
            })
    ...
    let result = await notificationModel.createNotification(notification);
